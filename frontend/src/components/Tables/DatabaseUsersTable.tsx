...
const DatabaseUsersTable = () => {
    ...
    const requestSort = (key: UserKey) => {
        setSortConfig(currentSortConfig => {
            // If the current key is clicked again and it's already in 'descending' order,
            // reset the sort configuration.
            if (currentSortConfig.key === key && currentSortConfig.direction === 'descending') {
                return { key: null, direction: 'ascending' }; // Resetting to default state
            }
            // Toggle between ascending and descending for the current key.
    ...
    const sortedUsers = React.useMemo(() => {
        ...
        if (sortConfig.key) {
            sortableUsers.sort((a, b) => {
                let valueA = typeof a[sortConfig.key] === 'string' ? a[sortConfig.key]?.toLowerCase() : a[sortConfig.key];
                let valueB = typeof b[sortConfig.key] === 'string' ? b[sortConfig.key]?.toLowerCase() : b[sortConfig.key];
