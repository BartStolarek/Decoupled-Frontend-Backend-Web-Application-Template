...
def setup_general():
    ...
    if user_query.first() is not None:
        if User.query.filter_by(email=Config.FAKE_EMAIL).first() is None:
            user = User(
                first_name=Config.FLASK_CONFIG,
                last_name='Fake',
...
def recreate_db():
    ...
    production.
    """
    response = input("Are you sure you want to drop all data, perhaps migrate is better? (y/n): ")
    if response.lower() == 'y':
        db.drop_all()
        db.create_all()
        db.session.commit()
...
@manager.option('-e', '--email', dest='email', help='The user email to be updated')
@manager.option('--first_name', dest='first_name', help='The first name to be set')
@manager.option('--last_name', dest='last_name', help='The last name to be set')
@manager.option('--password', dest='password', help='The password to be set')
@manager.option('--role_id', dest='role_id', default=1, help='The role_id to be set')
def add_user(email, first_name, last_name, password, role_id, arb_email_alerts, posEV_email_alerts):
...
@manager.option('-e', '--email', dest='email', help='The user email to be updated')  
@manager.option('-a', '--attribute', dest='attribute', help='The attribute to be updated')
...
def update_user(email, attribute, value):
    ...
    with app.app_context():
        ...
        if attribute == 'password':
            user_input = input("Changing the password might not include the password hash, check it was applied. Otherwise delete user and create a new one. Continue (y/n)?")
            if user_input == 'n':
                return
...
@manager.option('-f', '--filename', dest='filename', default=None, help='The file name to be tested')
